...
describe('v1.ClusterControllerClient', () => {
    ...
    describe('stopCluster', () => {
        it('invokes stopCluster without error', async () => {
            ...
            client.initialize();
            const request = generateSampleMessage(new protos.google.cloud.dataproc.v1.StopClusterRequest());
            request.projectId = '';
            request.region = '';
            request.clusterName = '';
            const expectedHeaderRequestParams = "project_id=&region=&cluster_name=";
            ...
            const [response] = await operation.promise();
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.stopCluster as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    ...
    describe('startCluster', () => {
        it('invokes startCluster without error', async () => {
            ...
            const [response] = await operation.promise();
            assert.deepStrictEqual(response, expectedResponse);
            assert((client.innerApiCalls.startCluster as SinonStub)
                .getCall(0).calledWith(request, expectedOptions, undefined));
        });
    ...
    describe('listClusters', () => {
        ...
        it('uses async iteration with listClusters with error', async () => {
            ...
            request.projectId = '';
            request.region = '';
            const expectedHeaderRequestParams = "project_id=&region=";const expectedError = new Error('expected');
